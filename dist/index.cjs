...
__export(index_exports, {
  CfbWriter: () => CfbWriter,
  MapiType: () => MapiType,
  NamedPropertyStreamName: () => NamedPropertyStreamName,
  PidTag: () => PidTag,
  RecipientTypeValue: () => RecipientTypeValue,
  createOft: () => createOft,
  createPropertiesStream: () => createPropertiesStream,
  decodePropertiesStream: () => decodePropertiesStream,
  encodePropertyValue: () => encodePropertyValue,
  htmlToText: () => htmlToText,
  inspectOft: () => inspectOft,
  listCfbEntries: () => listCfbEntries,
  normalizeHtml: () => normalizeHtml,
  patchOftTemplate: () => patchOftTemplate,
  readCfbStream: () => readCfbStream,
  recipientStorageName: () => recipientStorageName,
  substgStreamName: () => substgStreamName,
  writeEmptyNamedPropertyMappingStorage: () => writeEmptyNamedPropertyMappingStorage
});
...
var CFB2 = __toESM(require("cfb"), 1);
...
function headerSizeForKind(kind) {
...
function decodePropertiesStream(stream, kind) {
  ...
  for (let offset = headerSize; offset < stream.length; offset += 16) {
    ...
    const type = tag & 65535;
    const id = tag >>> 16 & 65535;
    const data = stream.subarray(offset + 8, offset + 16);
    const variableLength = type === MapiType.PT_UNICODE || type === MapiType.PT_STRING8 || type === MapiType.PT_BINARY;
    entries.push({
      propertyTag: tag,
      id,
      type,
      ...
      byteCount: variableLength ? stream.readUInt32LE(offset + 8) : void 0
    });
  }
  return entries;
}
function upsertVariableLengthPropertyEntry(cfb, streamPath, prop, kind) {
  if (!isVariableLengthProperty(prop)) {
    throw new Error("Only variable-length properties can be upserted with this helper.");
  }
  const existing = CFB2.find(cfb, streamPath);
  const valueLength = encodePropertyValue(prop).length;
  const newEntry = encodePropertyStreamEntry(prop);
  if (!existing?.content) {
    const stream2 = createPropertiesStream([prop], { kind });
    CFB2.utils.cfb_add(cfb, streamPath, stream2);
    return;
  }
...
// src/patch-oft-template.ts
var CFB4 = __toESM(require("cfb"), 1);

// src/cfb/cfb-reader.ts
var CFB3 = __toESM(require("cfb"), 1);
function parseCfb(buffer) {
  return CFB3.read(buffer, { type: "buffer" });
}
...
function readCfbStream(buffer, name) {
  const cfb = parseCfb(buffer);
  const root = cfb.FullPaths[0] ?? "";
  const entry = CFB3.find(cfb, name) ?? cfb.FileIndex.find((fileEntry, index) => stripRoot(cfb.FullPaths[index] ?? "", root) === name);
  if (!entry?.content) return null;
  return Buffer.from(entry.content);
}
function readCfbContainer(buffer) {
  return parseCfb(buffer);
}

// src/patch-oft-template.ts
function upsertStream(cfb, name, content) {
  const existing = CFB4.find(cfb, name);
  if (existing) {
    existing.content = content;
    existing.size = content.length;
    return;
  }
  CFB4.utils.cfb_add(cfb, name, content);
}
async function patchOftTemplate(baseOft, input) {
  if (baseOft.byteLength === 0) {
    throw new Error("patchOftTemplate requires a non-empty baseOft buffer.");
  }
  if (input.subject === void 0 && input.html === void 0 && input.text === void 0) {
  ...
  if (input.subject !== void 0) {
    const prop = { id: PidTag.Subject, type: MapiType.PT_UNICODE, value: input.subject };
    upsertStream(
      cfb,
  ...
  if (input.text !== void 0) {
    const prop = { id: PidTag.Body, type: MapiType.PT_UNICODE, value: input.text };
